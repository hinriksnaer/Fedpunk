...
function env-generate-fish-config
    # Generate Fish config file with all environment variables
    ...
    set -a config_lines "# Environment variables from modules and user config"
    set -a config_lines ""
    ...
    if test (count $seen_vars) -gt 0
        for pair in $var_values
            ...
            # For values with $FEDPUNK_PARAM_*, generate unquoted so Fish expands
            if string match -q '*$FEDPUNK_PARAM_*' "$value"
                set -a config_lines "set -gx $key $value"
            else
                ...
                set -a config_lines "set -gx $key \"$escaped_value\""
            end
        end
    end
