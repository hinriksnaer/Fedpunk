        ...
        if test "$use_multiline" = "true"
            ...
            while kill -0 $cmd_pid 2>/dev/null
                if test -f "$tmp_out" -a -s "$tmp_out"
                    ...
                    # Get last N lines, keep only text after final \r on each line, strip ANSI codes
                    set -l lines_to_show (tail -n $tail_lines "$tmp_out" 2>/dev/null | sed "s/.*\r//g" | sed "s/\x1b\[[0-9;]*[a-zA-Z]//g")
                    set -l count 0
