#!/usr/bin/env fish
# Module listing subcommands

function list --description "List modules"
...
function available --description "List available modules"
    ...
    set -l installed_modules (fedpunk-config-list-enabled-modules 2>/dev/null | while read -l ref
    ...
    if test -d "$FEDPUNK_SYSTEM/modules"
        for module in $FEDPUNK_SYSTEM/modules/*/
            if test -f "$module/module.yaml"
                ...
                if not contains "$name" $installed_modules
