...
│   └── dev/                     ← Your customizations (gitignored)
│       ├── plugins/             ← Custom modules
│       └── modes/               ← Profile modes
├── lib/fish/                    ← Core Fish libraries
└── cli/                         ← Core CLI commands

~/.config/                       ← XDG config directory (symlinked)
├── fish/                        ← Fish shell config
│   └── conf.d/
│       └── fedpunk-module-params.fish  ← Auto-generated parameters
...
~/.fedpunk/                      ← Runtime data
└── cache/
    └── external/                ← Cached external modules
```
...
Modules can define parameters that are injected as environment variables.

**Parameter Configuration:**
**Location:** `~/.config/fish/conf.d/fedpunk-module-params.fish` (auto-generated)

**Example:**
```fish
# Auto-generated by fedpunk - DO NOT EDIT
# Regenerate with: fedpunk install

# Parameters for: jira-tickets
set -gx FEDPUNK_PARAM_JIRA_TICKETS_TEAM_NAME "platform"
set -gx FEDPUNK_PARAM_JIRA_TICKETS_JIRA_URL "https://company.atlassian.net"
```
...
**Accessing in Module:**
```fish
# cli/my-module/my-module.fish
echo $FEDPUNK_PARAM_MY_MODULE_API_ENDPOINT
echo $FEDPUNK_PARAM_MY_MODULE_MAX_RETRIES
```

**Naming Convention:**
- Pattern: `FEDPUNK_PARAM_<MODULE>_<PARAM>`
- Module name from `module.yaml` (not repo name)
...
modules:
  ...
  # Local path (relative to home)
  - ~/gits/my-module
```

**Cache Location:** `~/.fedpunk/cache/external/<normalized-url>/`
...
**Lifecycle:**
1. During `fedpunk install`, external modules are fetched
2. Modules cached to `~/.fedpunk/cache/external/`
3. Parameters generated to Fish config
4. Modules deployed like built-in modules

### Profile Plugins

**Location:** `profiles/<profile>/plugins/<plugin-name>/`
